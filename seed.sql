...
SELECT id, title, duration FROM movies WHERE title = 'X-Men: Apocalypse';
SELECT id, title, duration FROM movies WHERE title = 'The Princess Bride';
SELECT id, title, released_at FROM movies ORDER BY released_at DESC;
SELECT id, title, genre, score FROM movies WHERE rating = 'PG' AND score BETWEEN 7.5 AND 8;
UPDATE movies SET score = score + 0.5 WHERE genre = 'CRIME';
UPDATE movies SET score = score - 2 WHERE is_3d IS TRUE;
